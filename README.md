...
```sh
git clone https://github.com/Atriarch-Systems/tracery-graph.git   # add --branch v0.1.0 for the tagged source release
cd tracery-graph
npm ci
npm run build
```

The Python client installs from the same checkout (Python >=3.11, no dependencies):
...
Start the hub from the repository root. With no environment variables it listens on `127.0.0.1:8971`, stores events in memory, and runs in local mode: one `default` workspace and no API key.
...
Node 22 also prints an `ExperimentalWarning` for `node:sqlite` at startup; it does not affect the memory store.
...
The same thing from the TypeScript client, with a subagent flow linked to the parent. Save this as `trace-demo.mjs` in the repository root of a built checkout, so the workspace package resolves:
...
From Python, after `python -m pip install ./clients/python`, save this as `trace_demo.py`:
...
Images are not on Docker Hub yet; build one from the repository root first:
...
[`apps/hub/helm`](apps/hub/helm/README.md) is a Helm chart (Deployment, Service, optional Ingress and ServiceMonitor, keys inline or from an existing Secret), and [`apps/hub/k8s`](apps/hub/k8s) has plain manifests. Both default to SQLite on a PVC with one replica; the chart refuses `replicaCount > 1` unless `config.store` is `postgres`. Until images are published, push a locally built image to your registry and set `image.repository` and `image.tag`.
...
For development, point Claude Code at the directory instead: `claude --plugin-dir ./plugins/claude-code`. With a local hub running (`node apps/hub/bin/hub.mjs`), the only setting needed is the hub URL.
