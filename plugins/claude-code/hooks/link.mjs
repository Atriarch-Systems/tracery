#!/usr/bin/env node
// Prints the Tracery Graph hub deep link for a session id. Used by the
...
const hubUrl = firstNonEmpty(env.CLAUDE_PLUGIN_OPTION_HUB_URL, env.TRACERY_HUB_URL).replace(/\/+$/, '');
const sessionId = process.argv[2];

if (hubUrl.length === 0) {
  console.log('Tracery Graph is not configured (no hub_url set for this plugin).');
} else if (!sessionId) {
