// Shared store-contract assertions (SPEC.md §6 "Storage"), run against every
// `EventStore` implementation. `registerStoreContractSuite(name, create)` is
// called once per engine: `store-contract.test.mjs` calls it for
// `MemoryStore` and `SqliteStore` (via `storeEngines` in `./helpers.mjs`);
// `postgres.test.mjs` calls it for `PostgresStore` (when docker is
// available). Keeping the assertions in one place is what stops the three
// implementations from quietly drifting apart on behaviour.
import test from 'node:test';
import assert from 'node:assert/strict';
import { performance } from 'node:perf_hooks';
import { evt, withStore } from './helpers.mjs';

export function startEvt(overrides) {
  return evt({ type: 'start', root: true, ...overrides });
}
export function endEvt(overrides) {
  return evt({ type: 'end', status: 'success', ...overrides });
}

export function registerStoreContractSuite(name, create) {
  ...
  test(`${name}: after a sweep evicts a flow, a reconnect from an older cursor gets a truncated snapshot`, () =>
    withStore(create, async (store) => {
      ...
      // hub-13 boundary: a client sitting exactly at the eviction boundary (one
      // cursor short of the oldest retained event) has lost nothing -- the next
      // event it needs is still there -- so it must get a plain delta, not a
      // spurious truncated snapshot.
      const retainedCursors = [...(await store.workspaceFrame('ws')).events].map((e) => e.cursor).sort((a, b) => a - b);
      const floorCursor = retainedCursors[0];
      const atBoundary = await store.workspaceFrame('ws', floorCursor - 1);
      assert.equal(atBoundary.type, 'events', `expected a plain delta at the eviction boundary, got a ${atBoundary.type}`);
    }));

  test(`${name}: a retention sweep never deletes a partial flow (no root start yet) that is still inside the retention window (hub-1)`, () =>
    withStore(create, async (store) => {
      const now = Date.now();
      // An update with no preceding start: SPEC.md §1 explicitly supports this --
      // it creates the op with `startedAt = undefined` and marks the flow `partial`.
      await store.append('ws', [
        { v: 1, id: 'late-child-1', ts: now, flow: 'late-child', op: 'o1', node: 'n1', type: 'update', name: 'x', context: { step: 1 } },
      ]);

      let summary = await store.flowSummary('ws', 'late-child');
      assert.equal(summary.partial, true);
      assert.equal(summary.startedAt, undefined);
      assert.notEqual(summary.status, 'complete');

      const result = await store.sweep(now, { retentionMs: 72 * 60 * 60 * 1000, maxEventsPerWorkspace: 500_000 });
      assert.equal(result.sweptFlows, 0, 'a brand-new partial flow must not be swept on the very next tick');

      summary = await store.flowSummary('ws', 'late-child');
      assert.notEqual(summary, undefined);

      // Its late-arriving root start is still accepted and re-projects the flow (SPEC.md §1).
      await store.append('ws', [startEvt({ id: 'late-child-0', flow: 'late-child', op: 'o1', ts: now - 5 })]);
      summary = await store.flowSummary('ws', 'late-child');
      assert.equal(summary.partial, false);
      assert.equal(summary.startedAt, now - 5);
    }));

  test(`${name}: a link cycle resolves without hanging, breaking to the first flow seen as the root (hub-2 incremental trace resolution)`, () =>
    withStore(create, async (store) => {
      await store.append('ws', [startEvt({ id: 'a1', flow: 'cycle-a', op: 'oa', ts: 1000, link: { parentFlow: 'cycle-b' } })]);
      await store.append('ws', [startEvt({ id: 'b1', flow: 'cycle-b', op: 'ob', ts: 1010, link: { parentFlow: 'cycle-a' } })]);

      const a = await store.flowSummary('ws', 'cycle-a');
      const b = await store.flowSummary('ws', 'cycle-b');
      assert.equal(a.trace, 'cycle-a');
      assert.equal(b.trace, 'cycle-a');
    }));

  test(`${name}: append cost does not grow with the number of already-retained events (hub-2)`, () =>
    withStore(create, async (store) => {
      const timeAppend = async (n) => {
        const started = performance.now();
        await store.append('ws', [startEvt({ id: `timing-${n}`, flow: `timing-flow-${n}`, op: 'o', ts: n, root: true })]);
        return performance.now() - started;
      };

      const empty = await timeAppend(0);

      const flowCount = 1500;
      for (let i = 0; i < flowCount; i++) {
        await store.append('ws', [startEvt({ id: `seed-${i}-s`, flow: `seed-flow-${i}`, op: 'o', ts: 1000 + i, root: true })]);
        await store.append('ws', [endEvt({ id: `seed-${i}-e`, flow: `seed-flow-${i}`, op: 'o', ts: 1000 + i + 1 })]);
      }

      const warm = await timeAppend(1);
      // A store that re-reduces the whole retained event log on every append
      // (the pre-hub-2 behaviour) makes `warm` scale with the ~3000 events just
      // seeded; an incremental one costs roughly the same as `empty` regardless
      // of how much history is already retained. Generous multiplier + floor to
      // stay stable on a loaded CI box.
      assert.ok(
        warm < Math.max(empty * 25, 75),
        `append took ${warm.toFixed(2)}ms with ${flowCount * 2} events already retained vs ${empty.toFixed(2)}ms on an empty store -- looks like it is re-reducing the whole event log on every append again`,
      );
    }));
}
